package br.com.influence.influence.handler;
...
import br.com.influence.influence.common.ConverterData;
...
public class RestExceptionHandler {
    ...
    public ResponseEntity<ErrorResponse> MethodNotSupportedException(MethodNotSupportedException ex) {
        ...
        ErrorResponse erro = new ErrorResponse(405, "Method Not Allowed", ex.getMessage(), ConverterData.converter(new Date()));
    ...
    public ResponseEntity<ErrorResponse> handleMyIOException(MyIOException ex) {
        ...
        ErrorResponse erro = new ErrorResponse(400, "I/O error while reading input ", ex.getMessage(), ConverterData.converter(new Date()));
    ...
    public ResponseEntity<ErrorResponse> handleHttpMessageNotReadable(HttpMessageNotReadableException ex) {
        ...
        if (cause instanceof MyIOException || cause instanceof JsonMappingException) {
            ...
            ErrorResponse erro = new ErrorResponse(400, "JSON Deserialize error", cause.getMessage(), ConverterData.converter(new Date()));

            return new ResponseEntity<>(erro, HttpStatus.BAD_REQUEST);
        }

        ErrorResponse erro = new ErrorResponse(500, "Internal Server Error", cause.getMessage(), ConverterData.converter(new Date()));

...
import { Influenciador, Marca } from "../../interface/index.tsx";
...
export const MarcasInfluencers: React.FC = () => {
  ...
  const [
    influenciadoresFiltrados, setInfluenciadoresFiltrados
  ] = useState<Influenciador[]>([])
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await getEmpresasPorToken(header);
        const empresasData: Marca = response.data;

        const influenciadoresData: Influenciador[] = empresasData.campanhas
          .flatMap((campanha) => campanha.campanhaInfluenciadores.map((ci) => ci.influenciador))
          .filter((influenciador): influenciador is Influenciador => influenciador !== undefined);
      ...
      <InfluenciadoresContent>
        <InfluenciadoresContainer>
          ...
          <ContainerSearch>
            ...
            <ButtonsContainer>
              <ButtonNeutro
                label="Botão para abrir as opções de filtro"
